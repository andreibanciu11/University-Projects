...
public class MyScanner {
    ...
    public void scan() {
        List<KeyValuePair<String, Integer>> tokenPairs = new ArrayList<>();
        try {
            ...
            while (reader.hasNextLine()) {
                ...
                for (String token : tokens) {
                    tokenPairs.add(new KeyValuePair<>(token, lineNr));
                }
    ...
    public String identifyStringConstant(String line, int position) {
        ...
        for (int i = position; i < line.length(); ++i) {
            if (((i == line.length() - 2 && line.charAt(i + 1) != '\"') || (i == line.length() - 1)))
                break;
    ...
    public void buildPIF(List<KeyValuePair<String, Integer>> tokens) {
        List<String> invalidTokens = new ArrayList<>();
        boolean isLexicallyCorrect = true;
        for (KeyValuePair<String, Integer> tokenPair : tokens) {
            ...
            if (ls.isOperator(token) || ls.isReservedWord(token) || ls.isSeparator(token)) {
                ...
                pif.add(code, new KeyValuePair<>(-1, -1));
            } else if (ls.isIdentifier(token)) {
                symbolTable.put(token);
                KeyValuePair<Integer, Integer> position = symbolTable.getPosition(token);
                pif.add(0, position);
