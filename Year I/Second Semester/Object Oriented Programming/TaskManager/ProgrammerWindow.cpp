...
ProgrammerWindow::ProgrammerWindow(Service serv, Programmer& p, QWidget* parent)
...
void ProgrammerWindow::handleStart()
{
	...
	if (t.getStatus() == "open")
	{
		this->serv.updateTaskStatus(t, "in progress", this->p.getId());
...
void ProgrammerWindow::handleDone()
{
	...
	ss >> t;
	int id = t.getId();
	this->serv.updateTaskStatus(t, "closed", id);
}
...
void ProgrammerWindow::updateTaskList()
{
	...
	for (auto t : this->serv.getTasks())
	{
		string s = t.toString();
		if (t.getId() != 0)
		{
			Programmer currentProgrammer = this->serv.getProgrammerById(t.getId());
